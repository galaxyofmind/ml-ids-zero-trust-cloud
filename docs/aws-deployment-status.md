...
| Model Registry | `bigdata-ids-dev-rf/3`, `bigdata-ids-dev-classical/3` | RF/SVM packages `Completed` and `Approved`; v1/v2 packages `Rejected` after inference boot failures |
| Online inference | `bigdata-ids-dev-rf` | First two Serverless attempts failed to boot; re-creating from corrected RF v3, check status below |
